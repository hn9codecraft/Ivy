...
define( 'LEARNDASH_ELEMENTOR_VERSION', '1.0.11' );
...
define( 'LEARNDASH_ELEMENTOR_ADMIN_VIEWS_DIR', plugin_dir_path( __FILE__ ) . 'src/admin-views/' );
define( 'LEARNDASH_ELEMENTOR_ADMIN_VIEWS_URL', plugin_dir_url( __FILE__ ) . 'src/admin-views/' );
...
use LearnDash\Elementor\StellarWP\Templates\Config as Templates_Config;
...
// Dependency checker message must be set at `init` action at the earliest to avoid the `_load_textdomain_just_in_time` warning.
add_action(
	'init',
	static function () use ( $learndash_elementor_dependency_checker ) {
...
add_action(
	...
	function () use ( $learndash_elementor_dependency_checker ) {
		...
		Templates_Config::set_hook_prefix( 'learndash_elementor' );
		Templates_Config::set_path( LEARNDASH_ELEMENTOR_PLUGIN_DIR );
